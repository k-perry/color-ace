...
export default class extends Component {
	...
	onRGBChange = (e) => {
		...
		this.setState({ [name]: val }, () => {
			const rgb = Utils.fixRGB([ this.state.red, this.state.green, this.state.blue ]);
	...
	onHSLChange = (e) => {
		...
		const name = e.target.name;
		this.setState({ [name]: val }, () => {
			const hsl = Utils.fixHSL([ this.state.hue, this.state.sat, this.state.lum ]);
			this.setColor(hsl, 'hsl');
		});
	};

	onHSLCopy = () => {
		const hsl = `hsl(${this.state.hue}, ${this.state.sat}%, ${this.state.lum}%)`;
		Utils.copyToClipboard(hsl);
	};

	onSave = () => {
		const newColor = { hex: this.state.hex, rgb: [ this.state.red, this.state.green, this.state.blue ] };
			...
			<div className="card">
				...
				<div style={{ display: 'flex', justifyContent: 'space-between' }}>
					<div style={{ width: '280px' }}>
						...
						/>
						<ColorPreview color={this.state.hex} shouldUpdate={this.state.isHexValid} />
