			...
			navigator.clipboard
				...
				.then((clipText) => {
					if (isValidHexColor(clipText)) {
						this.setState({ pasteDisabled: false, clipboardText: clipText });
					} else {
			...
			<div style={{ margin: '0.5rem 0.5rem 0 0.5rem' }}>
				...
				<input
					...
					style={{
						width: '75px',
						margin: '0 0.1rem',
						backgroundColor: this.props.isHexValid ? 'white' : '#FF8080'
					}}
				/>
				...
				<button
					...
					data-tip={"Paste '" + this.state.clipboardText + "' from clipboard"}
				>
