	...
	mapRow = (color, index) => {
		return (
			<tr key={index} onClick={this.onClick}>
				<td style={{ backgroundColor: color.hex }} data-tip="Color Preview" />
	...
	render() {
		return (
			<div className="card">
				...
				<div style={{ maxHeight: 'calc(100vh - 490px)', overflow: 'auto' }}>
					<table className="colors-table" style={{ width: '100%', height: '100%' }}>
						<thead>
							<tr>
								<th>Preview</th>
								<th>Hex</th>
						...
						<tbody className="monospace">
							{this.props.colors !== undefined ? (
								<React.Fragment>{this.props.colors.map(this.mapRow)}</React.Fragment>
							) : null}
						</tbody>
					</table>
				</div>
